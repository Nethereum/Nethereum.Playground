namespace Nethereum.Playground.Components.Sqlite
{
    using System;
    using System.IO;
    ...
    using WebAssembly;
    using SQLitePCL;
    // NOTE THIS IS a mock up at the moment to allow for autocompletion
    // This needs to be hooked to the Blazor JS runtime
    // and sqlite3 wasm in scripts etc.
    public class SQLite3Provider_WebAssembly : ISQLite3Provider
        {
            public int sqlite3_open(string filename, out IntPtr db)
            {
                var res = InvokeJSString($"SQLiteWasm.sqlite3_open(\"{Path.GetFileName(filename)}\")");
            ...
            {
                var res = InvokeJSString($"SQLiteWasm.sqlite3_open_v2(\"{Path.GetFileName(filename)}\", {flags}, {vfs})");
            ...
            public string sqlite3_db_filename(IntPtr db, string att)
                => InvokeJSString($"SQLiteWasm.sqlite3_db_filename({db}, \"{att}\")");

            public int sqlite3_changes(IntPtr db)
                => InvokeJSInt($"SQLiteWasm.sqliteChanges({db})");

            public string sqlite3_errstr(int rc)
            {
            ...
            public int sqlite3_prepare_v2(IntPtr db, string sql, out IntPtr stmt, out string remain)
            {
                ...
                var res = InvokeJSString($"SQLiteWasm.sqlitePrepare2({db}, \"{sql}\")");
            ...
            {
                var res = InvokeJSString($"SQLiteWasm.sqliteLastInsertRowid({db})");
            ...
            public string sqlite3_errmsg(IntPtr db)
                => InvokeJSString($"SQLiteWasm.sqliteErrMsg({db})");
            ...
            public int sqlite3_bind_text(IntPtr stmt, int index, string text)
                => InvokeJS($"SQLiteWasm.sqliteBindText({stmt}, {index}, \"{text}\")");
            ...
            public string sqlite3_column_name(IntPtr stmt, int index)
                => InvokeJSString($"SQLiteWasm.sqliteColumnName({stmt}, {index})");
            ...
            public string sqlite3_column_text(IntPtr stmt, int index)
                => InvokeJSString($"SQLiteWasm.sqliteColumnString({stmt}, {index})");
            ...
            public int sqlite3_column_bytes(IntPtr stmt, int index)
                => InvokeJSInt($"SQLiteWasm.sqlite3_column_bytes({stmt}, {index})");
            ...
            public string sqlite3_bind_parameter_name(IntPtr stmt, int index)
                => InvokeJSString($"SQLiteWasm.sqlite3_bind_parameter_name({stmt}, {index})");
            ...
            public void sqlite3_commit_hook(IntPtr db, delegate_commit func, object v) => throw new NotImplementedException();
               ...
               public string sqlite3_compileoption_get(int n)
               {
                   throw new NotImplementedException();
               }
            ...
            public byte[] sqlite3_value_blob(IntPtr p)
            {
                throw new NotImplementedException();
            }
            ...
            public string sqlite3_value_text(IntPtr p)
            {
                throw new NotImplementedException();
            }
            ...
            private static string InvokeJSString(string statement)
            {
